...
# 5. process -------------------------------------------------------------- TODO:
setwd("/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/xiao_plot/overlap_KD")
...
data_for_plot <- data.frame(group = c(rep("3D7", 500), rep("KD", 500)), order = c(1:500, 1:500), value = c(wt_line_data, kd_line_data))
...
ggplot(data_for_plot, aes(x = order, y = value, group = group, color = group)) +
    ...
    scale_fill_manual(values = c("KD" = "red", "3D7" = "black"), , guide = "none") +
    scale_x_continuous(breaks = c(0, 100, 250, 400, 500), labels = c("2kb\nupstream", "Start", "gene body", "Stop", "2kb\ndownstream")) +
    scale_color_manual(values = c("KD" = "red", "3D7" = "black"), ) +
...
ggsave("overlapgene_KD.pdf", width = 10, height = 7)

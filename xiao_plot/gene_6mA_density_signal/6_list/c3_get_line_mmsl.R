...
read.data <- function(part = NULL, type = NULL) {
    ...
    if (type == "wt") {
        chip_depth <- 45844430
        input_depth <- 36605597
    } else if (type == "kd") {
        chip_depth <- 33789613
        input_depth <- 25724461
    }
...
# 5. process -------------------------------------------------------------- TODO:
setwd(paste0("/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/xiao_plot/6_list/", prefix, "/mmsL"))
...
data_for_plot <- data.frame(group = c(rep("3D7", 500), rep("MMSL", 500)), order = c(1:500, 1:500), value = c(wt_line_data, kd_line_data))
# data_for_plot$order<-factor(data_for_plot$order,1:300)

pdf(paste0(prefix, ".mmsl.pdf"), width = 10, height = 7)
ggplot(data_for_plot, aes(x = order, y = value, group = group, color = group)) +
    # geom_line()
    # geom_point() +
    geom_smooth(method = "loess", span = 0.3, se = FALSE, size = 3) +
    scale_fill_manual(values = c("MMSL" = "red", "3D7" = "black"), , guide = "none") +
    scale_x_continuous(breaks = c(0, 100, 250, 400, 500), labels = c("2kb\nupstream", "Start", "gene body", "Stop", "2kb\ndownstream")) +
    scale_color_manual(values = c("MMSL" = "red", "3D7" = "black"), ) +

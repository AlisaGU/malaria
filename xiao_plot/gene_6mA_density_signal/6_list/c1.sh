...
MMS_bam_dir="/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/xiao_plot/MMS"
...
WT_chip=$MMS_bam_dir/3D7-0-3_ChIP.bam
WT_input=$MMS_bam_dir/3D7-0-3_Input.bam
KD_chip=$MMS_bam_dir/3D7-mmsL-1_ChIP.bam
KD_input=$MMS_bam_dir/3D7-mmsL-1_Input.bam
...
for prefix in $(echo "merged MMR NER BER MMEJ HDR" | tr " " "\n"); do
    echo $prefix
    working_dir=/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/xiao_plot/6_list/$prefix/mmsL
    mkdir -p $working_dir
    cd $working_dir
    sort -u ../${prefix}_geneList >../${prefix}_geneList.unique.mmsl
    grep -f ../${prefix}_geneList.unique.mmsl $genes | $bedtools makewindows -b stdin -n 300 -i srcwinnum >$prefix.gene.window
    grep -f ../${prefix}_geneList.unique.mmsl $TSS2kb | $bedtools makewindows -b stdin -n 100 -i srcwinnum >$prefix.tss.window
    grep -f ../${prefix}_geneList.unique.mmsl $TTS2kb | $bedtools makewindows -b stdin -n 100 -i srcwinnum >$prefix.tts.window

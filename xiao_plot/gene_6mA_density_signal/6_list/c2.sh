...
for prefix in $(echo "merged MMR NER BER MMEJ HDR" | tr " " "\n"); do
    echo $prefix
    working_dir=/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/xiao_plot/6_list/$prefix/T3
    mkdir -p $working_dir
    cd $working_dir
    sort -u ../${prefix}_geneList >../${prefix}_geneList.unique.t3
    grep -f ../${prefix}_geneList.unique.t3 $genes | $bedtools makewindows -b stdin -n 300 -i srcwinnum >$prefix.gene.window
    grep -f ../${prefix}_geneList.unique.t3 $TSS2kb | $bedtools makewindows -b stdin -n 100 -i srcwinnum >$prefix.tss.window
    grep -f ../${prefix}_geneList.unique.t3 $TTS2kb | $bedtools makewindows -b stdin -n 100 -i srcwinnum >$prefix.tts.window

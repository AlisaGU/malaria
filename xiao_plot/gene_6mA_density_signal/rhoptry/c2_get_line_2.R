...
# 5. process -------------------------------------------------------------- TODO:
setwd("/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/xiao_plot/OE/rhoptry_2")
...
ggplot(data_for_plot, aes(x = order, y = value, group = group, color = group)) +
    # geom_line()
    # geom_point() +
    geom_smooth(method = "loess", span = 0.1, se = FALSE, size = 3) +
...
ggsave("rhoptry_2.pdf", width = 10, height = 7)

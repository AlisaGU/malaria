...
# 5. process -------------------------------------------------------------- TODO:
setwd("/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/xiao_plot/OE/rhoptry_1")
...
ggplot(data_for_plot, aes(x = order, y = value, group = group, color = group)) +
    # geom_line()
    # geom_point() +
    geom_smooth(method = "loess", span = 0.2, se = FALSE, size = 3) +
...
ggsave("rhoptry_1.pdf", width = 10, height = 7)

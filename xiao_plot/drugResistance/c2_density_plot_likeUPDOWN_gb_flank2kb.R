...
subsample_bam_gene_density <- function(gene_list = NULL, data = NULL, color = NULL) {
    observed <- data.frame(value = data$gene_flank2kb_density_signal[match(gene_list, data$gene)])
...
data_drugResistance <- data %>%
    ...
    select(gene, gene_flank2kb_density_signal)
write.table(data_drugResistance, "/picb/evolgen2/users/gushanshan/projects/malaria/dataAndResult/xiao_plot/drugResistance/drug_resistance_gene_flank2kb_6mADensity.txt", row.names = F, col.names = T, quote = F, sep = "\t")

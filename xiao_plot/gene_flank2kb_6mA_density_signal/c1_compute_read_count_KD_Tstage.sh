...
get_gene_6mA_density_signal() {
    ...
    awk '{print $1"\t"$2-1"\t"$2"\t"$3}' $gene_depth_chip | $bedtools intersect -wa -a $all_genes_bed -wb -b stdin >$bam_dir/chip.gene_flank2kb.inter
    awk '{print $1"\t"$2-1"\t"$2"\t"$3}' $gene_depth_input | $bedtools intersect -wa -a $all_genes_bed -wb -b stdin >$bam_dir/input.gene_flank2kb.inter
}
...
gene_depth_chip=$bam_dir/gene_flank2kb_depth_chip
gene_depth_input=$bam_dir/gene_flank2kb_depth_input

...
get_gene_6mA_density_signal() {
    ...
    awk '{print $1"\t"$2-1"\t"$2"\t"$3}' $gene_depth_chip | $bedtools intersect -wa -a $all_genes_bed -wb -b stdin >$working_dir/NAMTOE-T_chip.gene_flank2kb.inter
    awk '{print $1"\t"$2-1"\t"$2"\t"$3}' $gene_depth_input | $bedtools intersect -wa -a $all_genes_bed -wb -b stdin >$working_dir/NAMTOE-T_input.gene_flank2kb.inter
...
chip_summary=$bam_dir/NAMTOE-T_ChIP.bam.summary
input_summary=$bam_dir/NAMTOE-T_Input.bam.summary

gene_depth_chip=$working_dir/NAMTOE-T_gene_flank2kb_depth_chip
gene_depth_input=$working_dir/NAMTOE-T_gene_flank2kb_depth_input
...
$samtools depth -a -b $all_genes_bed $bam_dir/NAMTOE-T_ChIP.bam >$gene_depth_chip
$samtools depth -a -b $all_genes_bed $bam_dir/NAMTOE-T_Input.bam >$gene_depth_input
get_gene_6mA_density_signal

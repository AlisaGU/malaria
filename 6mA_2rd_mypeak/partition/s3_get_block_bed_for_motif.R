...
write_bed_for_one_motif <- function(data = NULL, chrom = NULL, output_dir = NULL) {
    ...
    write.table(matrix(result$motif_loc, ncol = 3), file = motif_file, quote = F, row.names = F, col.names = F, sep = "\t", append = T)
    write.table(matrix(result$region_1_loc, ncol = 3), file = region1_file, quote = F, row.names = F, col.names = F, sep = "\t", append = T)
    write.table(matrix(result$region_2_loc, ncol = 3), file = region2_file, quote = F, row.names = F, col.names = F, sep = "\t", append = T)
    write.table(matrix(result$region_3_loc, ncol = 3), file = region3_file, quote = F, row.names = F, col.names = F, sep = "\t", append = T)
    write.table(matrix(result$region_4_loc, ncol = 3), file = region4_file, quote = F, row.names = F, col.names = F, sep = "\t", append = T)
}
# 3. input ----------------------------------------------------------------
Args <- commandArgs()
...
# motif_bed_file<-"/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/2_2rd_partition/intergenic_motif/motif_loc/Pf3D7_05_v3.motif.bed"
# output_dir<-"/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/2_2rd_partition/intergenic_motif/region_loc"
# 5. process --------------------------------------------------------------

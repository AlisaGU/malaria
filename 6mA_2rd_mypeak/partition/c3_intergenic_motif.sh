...
# FUNCTIONS TODO:
extract_peak_seq_in_intergenic_region() {
    mkdir -p $core_peak_chrom_dir
    cd $core_peak_chrom_dir

    for autosome in $(echo $autosomes | tr " " "\n"); do
        # $bedtools intersect -a $intergenic_dir/$autosome -b $core_peak_bed >${autosome}.peak.core.bed
        $seqkit subseq --bed ${autosome}.peak.core.bed $genome -o ${autosome}.peak.core.fa
    done
...
locate_motif() {
    ...
    for autosome in $(echo $autosomes | tr " " "\n"); do
        cat $core_peak_chrom_dir/${autosome}.peak.core.fa | $seqkit locate -i -d -f $motif_pattern --bed >${autosome}.motif.bed
    done
...
get_block_bed_for_motif() {
    ...
    for bed in $(ls $motif_bed_dir); do
        $code_dir/s3_get_block_bed_for_motif.R ${motif_bed_dir}/$bed $region_bed_dir
    done
}
...
get_average_variant_count_each_popu() {
    ...
    for chrom in $(echo $autosomes | tr " " "\n"); do
        ...
        regions="$intergenic_dir/../${chrom}.nopeak.intergic.bed"
...
popu_symbols="CAF"
code_dir="/picb/evolgen2/users/gushanshan/projects/malaria/code/6mA_2rd_mypeak/partition"
working_dir="/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/2_2rd_partition/intergenic_motif"
...
core_nopeak_bed_dir="/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/6mA/jiang/2rd/macs2_output"

core_peak_bed=$core_nopeak_bed_dir/"peak.core.bed"
core_nopeak_chrom_dir=$core_nopeak_bed_dir/chrom
core_peak_chrom_dir=$working_dir/peak_chrom
motif_bed_dir=$working_dir/motif_loc
motif_pattern=$working_dir/motif_pattern

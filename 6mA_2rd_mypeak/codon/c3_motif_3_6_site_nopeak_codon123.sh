...
get_motif_3_6_site_appear_in_codon123() {
    ...
    for chrom in $(echo $autosomes | tr " " "\n"); do
        ...
        codon1=$(awk '$3==1' ${cds_each_site_codon_index}.sort | awk '{print $1"\t"$2-1"\t"$2}' | $bedtools subtract -a stdin -b $two_outgroup_inconsistent_bed | $bedtools intersect -a stdin -b $motif_whole_genome_loc/${chrom}.nopeak.motif.3_6 | wc -l)
        codon1_total=$(awk -v chrom=$chrom '$1==chrom && $3==1' ${cds_each_site_codon_index}.sort | awk '{print $1"\t"$2-1"\t"$2}' | $bedtools subtract -a stdin -b $two_outgroup_inconsistent_bed | $bedtools intersect -a stdin -b $chrom_wholeGenome_dir/$chrom.nopeak.bed | wc -l)

        codon2=$(awk '$3==2' ${cds_each_site_codon_index}.sort | awk '{print $1"\t"$2-1"\t"$2}' | $bedtools subtract -a stdin -b $two_outgroup_inconsistent_bed | $bedtools intersect -a stdin -b $motif_whole_genome_loc/${chrom}.nopeak.motif.3_6 | wc -l)
        codon2_total=$(awk -v chrom=$chrom '$1==chrom && $3==2' ${cds_each_site_codon_index}.sort | awk '{print $1"\t"$2-1"\t"$2}' | $bedtools subtract -a stdin -b $two_outgroup_inconsistent_bed | $bedtools intersect -a stdin -b $chrom_wholeGenome_dir/$chrom.nopeak.bed | wc -l)

        codon3=$(awk '$3==3' ${cds_each_site_codon_index}.sort | awk '{print $1"\t"$2-1"\t"$2}' | $bedtools subtract -a stdin -b $two_outgroup_inconsistent_bed | $bedtools intersect -a stdin -b $motif_whole_genome_loc/${chrom}.nopeak.motif.3_6 | wc -l)
        codon3_total=$(awk -v chrom=$chrom '$1==chrom && $3==3' ${cds_each_site_codon_index}.sort | awk '{print $1"\t"$2-1"\t"$2}' | $bedtools subtract -a stdin -b $two_outgroup_inconsistent_bed | $bedtools intersect -a stdin -b $chrom_wholeGenome_dir/$chrom.nopeak.bed | wc -l)

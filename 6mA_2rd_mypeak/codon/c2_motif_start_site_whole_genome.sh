...
get_motif_first_site_appear_in_codon() {
    ...
    motif_each_site=$motif_start_site_dir/motif_each_site
    rm -rf $motif_each_site
    for chrom in $(echo $autosomes | tr " " "\n"); do
        awk '$6=="+"{print $1"\t"$2"\t"$2+1};$6=="-"{print $1"\t"$3-1"\t"$3}' $motif_whole_genome_loc/${chrom}.genome.motif.bed | sort -k1,1 -k2,2n >$motif_whole_genome_loc/${chrom}.motif.firstSite

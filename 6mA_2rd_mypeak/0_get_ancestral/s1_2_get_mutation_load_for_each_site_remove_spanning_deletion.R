...
Args <- commandArgs()
vcf_data <- Args[6]
output <- paste0(dirname(vcf_data), "/", gsub(".vcf.gz", "", basename(vcf_data)), "_correct_spanning_deletion_polysite.bed")
...
popu_notAllSameAsRef_index <- which(apply(vcf_ancestor_nomiss[, 5:ncol(vcf_ancestor_nomiss)], 1, function(popu) {
    popu_nomiss <- popu[popu != "."]
    any(popu_nomiss != 0)
}))
data_for_mutation_load <- vcf_ancestor_nomiss[popu_notAllSameAsRef_index, ] # 去除3D7和该群体的等位完全一致的位点

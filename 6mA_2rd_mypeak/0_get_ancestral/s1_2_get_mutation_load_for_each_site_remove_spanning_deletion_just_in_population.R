...
Args <- commandArgs()
vcf_data <- Args[6]
output <- paste0(dirname(vcf_data), "/", gsub(".vcf.gz", "", basename(vcf_data)), "_correct_spanning_deletion_polysite_just_in_population.bed")
...
mutation_load <- mclapply(gt, function(x) {
    ...
    allele_class <- unique(x[4:length(x)])

...
Args <- commandArgs()
vcf_data <- Args[6]
output <- paste0(dirname(vcf_data), "/", gsub(".vcf.gz", "", basename(vcf_data)), "_correct_spanning_deletion_polysite_remove_popu_monomorphism.bed")
...
vcf_ancestor_nomiss <- vcf
popu_notMonomorphism_index <- which(apply(vcf_ancestor_nomiss[, 5:ncol(vcf_ancestor_nomiss)], 1, function(popu) {
    popu_nomiss <- popu[popu != "."]
    # any(popu_nomiss != 0)
    length(unique(popu_nomiss)) > 1
}))
data_for_mutation_load <- vcf_ancestor_nomiss[popu_notMonomorphism_index, ] # 去除3D7和该群体的等位完全一致的位点
...
non_null_index <- which(sapply(mutation_load, function(x) {
    !is.null(x)
}))
mutation_load <- do.call(rbind, mutation_load)
result <- data.frame(chrom = chrom, bed0 = data_for_mutation_load$pos[non_null_index] - 1, bed1 = data_for_mutation_load$pos[non_null_index], mutation_load = mutation_load)

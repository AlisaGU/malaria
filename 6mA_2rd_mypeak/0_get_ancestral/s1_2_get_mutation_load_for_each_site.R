...
mutation_load <- mclapply(gt, function(x) {
    ...
    ts_direc<-sapply(c("A2G","G2A","T2C","C2T"),function(x){length(which(exist_allele_data$variant_types==x))})
    tv_direc<-sapply(c("A2C","A2T","G2C","G2T","C2A","C2G","T2A","T2G"),function(x){length(which(exist_allele_data$variant_types==x))})

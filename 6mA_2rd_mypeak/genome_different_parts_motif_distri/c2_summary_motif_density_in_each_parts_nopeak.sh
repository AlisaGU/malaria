...
compute_average_motif_count_and_base_count() {
    ...
    for chrom in $(echo $autosomes | tr " " "\n"); do
        ...
        for region in $(echo "intergenic TSS_2KB TTS_2KB genes exons introns" | tr " " "\n"); do
            ...
            motif_count=$($bedtools intersect -a $motif_whole_genome_loc/${chrom}.nopeak.genome.motif.bed -b $region_bed | wc -l | awk '{print $1}')
            motif_base_count=$($bedtools intersect -a $motif_whole_genome_loc/${chrom}.nopeak.genome.motif.bed -b $region_bed | sort -k1,1 -k2,2n | $bedtools merge -i stdin | awk '{print $3-$2}' | awk '{s+=$1}END{print s}')
            # region_len=$(awk -v chrom=$chrom '$1==chrom{print $3-$2}' $region_bed | awk '{s+=$1};END{print s}')
            region_len=$(awk -v chrom=$chrom '$1==chrom' $region_bed | $bedtools intersect -a stdin -b $chrom_wholeGenome_dir/${chrom}.nopeak.bed | awk '{s+=$3-$2}END{print s}')

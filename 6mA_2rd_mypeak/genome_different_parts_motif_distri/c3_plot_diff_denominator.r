...
get_plot <- function(filename = NULL) {
    ...
    p <- ggplot(data_for_plot %>% filter(class == "motif_base_count") %>% filter(source != "intergenic"), aes(x = source, y = value)) +
...
get_ratio_plot <- function() {
    ...
    p <- ggplot(data_for_plot_ratio %>% filter(class == "motif_base_count") %>% filter(source != "intergenic"), aes(x = source, y = value)) +
        geom_boxplot(aes(color = source), width = 0.5, size = 1) +
        geom_point(aes(color = source), position = position_jitterdodge(), size = 4) +
...
# 3. input ---------------------------------------------------------------- TODO:
setwd("/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/6mA/jiang/2rd/macs2_output/genome_different_parts_motif_distri/motif_density_diff_denominator")

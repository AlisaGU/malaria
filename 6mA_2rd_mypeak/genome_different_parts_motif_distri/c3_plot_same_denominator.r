...
read.data <- function(filename = NULL) {
    ...
    colnames(data) <- paste(rep(c("intergenic", "TSS_2KB", "TTS_2KB", "genes", "exons", "introns"), each = 3), rep(c("motif_count", "motif_base_count", "region_len"), 6), sep = ".")
    motif_count <- data[, seq(1, ncol(data), by = 3)]
    motif_base_count <- data[, seq(2, ncol(data), by = 3)]
    region_len <- data[, seq(3, ncol(data), by = 3)]

    motif_average_count <- motif_count / region_len
    motif_avergae_base_count <- motif_base_count / region_len

    a <- data.frame(
        value = unlist(motif_average_count),
        chrom = rep(rownames(motif_average_count), ncol(motif_average_count)),
        source = rep(gsub(".motif_count", "", colnames(motif_average_count)),
            each = nrow(motif_average_count)
        ),
        class = "motif_count"
    )
...
# 3. input ---------------------------------------------------------------- TODO:
setwd("/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/6mA/jiang/2rd/macs2_output/genome_different_parts_motif_distri/motif_density")

# 4. variable setting of test module--------------------------------------- TODO:


# 5. process -------------------------------------------------------------- TODO:
data_for_plot <- read.data(filename = "nopeak_motif_density")
ggplot(data_for_plot %>% filter(class == "motif_base_count"), aes(x = source, y = value)) +
    ...
    scale_x_discrete(labels = c("Intergenic\nregion", "TSS 2kb", "TTS 2kb", "Gene", "Exon", "Intron")) +
    labs(x = "", y = "Porportion of motif base") +
    theme_bw() +
    theme(
...
# ggsave("motif_density_in_genome_parts.pdf", width = 12, height = 7)
ggsave("mopeak_motif_base_count_proportion_in_genome_parts.pdf", width = 8, height = 10)

...
compute_average_motif_count_and_base_count() {
    density_file=$motif_density_dir/motif_density
    rm -rf $density_file
    ...
    for chrom in $(echo $autosomes | tr " " "\n"); do
        ...
        for region in $(echo "intergenic TSS_2KB TTS_2KB genes exons introns" | tr " " "\n"); do
            ...
            motif_count=$($bedtools intersect -a $motif_whole_genome_loc/${chrom}.genome.motif.bed -b $region_bed | wc -l | awk '{print $1}')
            motif_base_count=$($bedtools intersect -a $motif_whole_genome_loc/${chrom}.genome.motif.bed -b $region_bed | sort -k1,1 -k2,2n | $bedtools merge -i stdin | awk '{print $3-$2}' | awk '{s+=$1}END{print s}')
...
motif_whole_genome_loc="/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/6mA/jiang/2rd/macs2_output/chrom/motif_GAWGAW_genome_loc" ##虽然这个目录是在chrom下，但motif确实是全基因组的

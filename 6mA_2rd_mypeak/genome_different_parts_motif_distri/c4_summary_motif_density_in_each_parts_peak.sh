...
compute_average_motif_count_and_base_count() {
    ...
    for chrom in $(echo $autosomes | tr " " "\n"); do
        ...
        for region in $(echo "intergenic TSS_2KB TTS_2KB genes exons introns" | tr " " "\n"); do
            ...
            region_bed_chrom=$(awk -v chrom=$chrom '$1==chrom' $region_bed | $bedtools intersect -a stdin -b $chrom_core_genome_two_outgroup_peak_dir/${chrom}.peak.bed)

            motif_base_count=$(echo "$region_bed_chrom" | $bedtools intersect -a $motif_core_genome_two_outgroup_peak_dir/${chrom}.motif.right.bed -b stdin | sort -k1,1 -k2,2n | $bedtools merge -i stdin | awk '{print $3-$2}' | awk '{s+=$1}END{print s}')
...
motif_core_genome_two_outgroup_peak_dir="/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/6mA/jiang/2rd/macs2_output/two_outgroup_consistent/motif_GAWGAW/motif_loc"
chrom_core_genome_two_outgroup_peak_dir=/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/6mA/jiang/2rd/macs2_output/two_outgroup_consistent

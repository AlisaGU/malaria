...
get_average_variant_count_for_each_motif_global_variants() {
    ...
    mean_variant_count=$mutation_density_dir/site${site_index}.snps_mean_variant_count
    ...
    for chrom in $(echo $autosomes | tr " " "\n"); do
        ...
        for peak_type in $(echo "peak nopeak" | tr " " "\n"); do
            for base_status in $(echo "A T C G" | tr " " "\n"); do
                this_site_this_bases_count=$($seqkit subseq --quiet --bed $motif_site_dir/site${site_index}/${chrom}.${peak_type}.bed $genome | $seqkit fx2tab -i -H -n -C "${base_status}" | grep -v "^#" | awk '{s+=$2} END{print s}')
                this_site_this_bases_mutation_count=$(get_average_variant_count_for_chrom_base_count $chrom $motif_site_dir/site${site_index}/${chrom}.${peak_type}.bed "${base_status}_snps")

...
get_mutation_load_for_all_chroms() {
    for chrom in $(echo $chroms | tr " " "\n"); do
        sed "s/chrom_template/$chrom/g" $code_dir/s1_get_mutation_load_for_one_chrom.sh | sed "s/popu_symbol_template/$popu_symbol/g" >$code_dir/s1_get_mutation_load_for_one_chrom.sh_${chrom}

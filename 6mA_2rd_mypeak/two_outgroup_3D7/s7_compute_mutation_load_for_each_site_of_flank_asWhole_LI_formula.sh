...
get_average_variant_count_for_each_motif_global_variants() {
    ...
    mean_variant_count=$specific_mutation_density_dir/flank_${flank_site}.snps_mean_variant_count
    ...
    for chrom in $(echo $autosomes | tr " " "\n"); do
        ...
        for peak_type in $(echo "peak nopeak" | tr " " "\n"); do
            for base_status in $(echo "A T C G" | tr " " "\n"); do
                ...
                this_site_this_bases_mutation_count=$(get_average_variant_count_for_chrom_base_count $chrom $single_motif_output_dir/flank_${flank_type}/flank_${flank_site}/${chrom}.${peak_type}.bed "${base_status}_snps")
...
single_motif_global_output_dir=""

...
subset_only_VQSLOD_gt0_vcf_for_one_chrom() {
    chrom=$1
    variant_file=$pf6_variant_dir/"Pf_60_public_Pf3D7_${chrom}_v3.final.vcf.gz"
    popu_PASS_variant=$popu_variant_dir/"${popu_symbol}_VQSLOD_gt0_${chrom}.vcf.gz"

    $bcftools view --include 'VQSLOD>0.0' -S $popu_samplelist -O z -o $popu_PASS_variant --threads 4 $variant_file 1>$popu_variant_dir/$chrom"_VQSLOD_gt0__select_sample.log" 2>&1
    $tabix $popu_PASS_variant
}

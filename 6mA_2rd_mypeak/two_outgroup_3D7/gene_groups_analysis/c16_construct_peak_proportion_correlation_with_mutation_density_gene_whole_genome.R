...
read_peak_proportion_and_mutation_density <- function() {
    mutation_density <- read.table(paste0(mutation_density_global_dir, "/snps_mean_variant_count_gene"), as.is = T, stringsAsFactors = F)
    ...
    peak_length <- read.table(paste0(peak_proportion_global_dir, "/peak_control_length_in_gene"), as.is = T, stringsAsFactors = F)
...
noncore_p$labels$y <- ""
core_p | noncore_p
ggsave("/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/1_2rd_initial_evaluation/ESEA_WSEA_OCE_SAM_SAS/two_outgroup_consistent_in_core_and_noncore/wholeGenome/correlation_between_peak_proportion_with_MD_whole_genome_gene.pdf", width = 16, height = 8)

...
get_cds_locus() {
    ...
    grep -f ${gene_set}_genes_list $anno | awk '$3=="CDS"' | awk '{split($9,a,";");split(a[1],b,"=");print $1"\t"$4-1"\t"$5"\t"b[2]}' | sort -k1,1 -k2,2n >${gene_set}_cds.bed
}
...
get_peak_control_length_in_gene_for_each_gene_set_include_nopeak_gene_with_fold_enrichment() {
    ...
    cat $gene_sets_global_dir/$gene_set/${gene_set}_genes.bed | while read line; do
        peak_exist_in_gene_status=$(echo $"$line" | $bedtools intersect -a stdin -b $peak_with_fold_enrichment | $bedtools intersect -a stdin -b ${gene_set}_cds.bed | wc -l)
        value=""

        gene_name=$(echo $line | awk '{print $4}')
        cds_length_of_gene=$(grep $gene_name ${gene_set}_cds.bed | awk '{s+=$3-$2}END{print s}')

        cds_length_of_gene_in_peak=0
        if [ $peak_exist_in_gene_status -gt 0 ]; then
            cds_length_of_gene_in_peak=$(echo $"$line" | $bedtools intersect -a stdin -b $peak_with_fold_enrichment | $bedtools subtract -a stdin -b $inconsistent_region_in_core_and_noncore | $bedtools intersect -a stdin -b ${gene_set}_cds.bed | awk '{s+=$3-$2}END{print s}')
        fi
        let cds_length_of_gene_in_control=cds_length_of_gene-cds_length_of_gene_in_peak
        value+="$(echo $gene_name $cds_length_of_gene_in_peak $cds_length_of_gene_in_control)"

        echo $value >>$peak_control_length_in_gene
    done
}

export -f get_cds_locus get_peak_control_length_in_gene_for_each_gene_set_include_nopeak_gene_with_fold_enrichment
...
# PROCESS TODO:
set -ex
$parallel get_cds_locus ::: $gene_sets

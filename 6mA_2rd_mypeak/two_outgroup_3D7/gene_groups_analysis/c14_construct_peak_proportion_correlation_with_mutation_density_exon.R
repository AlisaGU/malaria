...
read_peak_proportion_and_mutation_density <- function() {
    result <- lapply(gene_sets, function(gene_set) {
        mutation_density <- read.table(paste0(mutation_density_global_dir, "/", gene_set, "/snps_mean_variant_count_exon"), as.is = T, stringsAsFactors = F)
        ...
        peak_length <- read.table(paste0(peak_proportion_global_dir, "/", gene_set, "/", gene_set, "_peak_control_length_in_exon"), as.is = T, stringsAsFactors = F)
...
noncore_p$labels$y <- ""
core_p | noncore_p
ggsave("/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/1_2rd_initial_evaluation/ESEA_WSEA_OCE_SAM_SAS/two_outgroup_consistent_in_core_and_noncore/gene_5_sets_exclude_pseudo_with_fold_enrichment_including_RIF/correlation_between_peak_proportion_with_MD.pdf", width = 16, height = 8)

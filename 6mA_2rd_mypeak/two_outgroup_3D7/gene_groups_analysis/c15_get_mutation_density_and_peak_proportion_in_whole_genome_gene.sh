...
get_mutation_density() {
    ...
    mean_variant_count=$mutation_density_dir/${var_type}_mean_variant_count_gene
...
get_peak_proportion() {
    ...
    peak_control_length_in_gene=peak_control_length_in_gene
    ...
    cat genes.bed | while read line; do
        ...
        gene_name=$(echo $line | awk '{print $4}')
        gene_length=$(echo "$line" | $bedtools subtract -a stdin -b $inconsistent_region_in_core_and_noncore | awk '{s+=$3-$2}END{print s}')
...
whole_genome_bed_dir="/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/gene_groups/whole_Genome"
...
gene_bed=$whole_genome_bed_dir/genes.bed
gene_flank_bed=$whole_genome_bed_dir/gene_flank.bed
...
# PROCESS TODO:
# set -ex
get_mutation_density "snps"
get_mutation_density "indels"

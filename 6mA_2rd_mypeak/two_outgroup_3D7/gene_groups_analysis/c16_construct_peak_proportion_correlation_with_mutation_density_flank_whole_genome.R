...
core_p <- plot_scatter(data_for_plot = data_core %>% filter(peak_proportion != 0) %>% filter(length >= 500)) + ggtitle("Euchromatin")
noncore_p <- plot_scatter(data_for_plot = data_noncore %>% filter(peak_proportion != 0) %>% filter(length >= 500)) + ggtitle("heterochromatin")
noncore_p$labels$y <- ""
core_p | noncore_p
ggsave("/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/1_2rd_initial_evaluation/ESEA_WSEA_OCE_SAM_SAS/two_outgroup_consistent_in_core_and_noncore/wholeGenome/correlation_between_peak_proportion_with_MD_whole_genome_flank.pdf", width = 16, height = 8)

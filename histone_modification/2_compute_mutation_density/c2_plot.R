...
get_data_for_plot_facet_H3K4me3_H3K9me3 <- function(dataname = NULL) {
...
plot_mutation_density <- function(data_for_plot = NULL, histone_type = NULL) {
    ...
    p <- ggplot(data_for_plot %>% filter(histone == histone_type) %>% filter(source != "nopeak500"), aes(x = source, y = value, color = source)) +
        geom_boxplot(position = position_dodge(), outlier.shape = NA, width = 0.5, size = 1) +
        geom_point(position = position_jitterdodge(), size = 6) +
        facet_wrap(~variant_f, nrow = 1, ncol = 2, scale = "free") +
...
histone_type <- c("H3K9me3", "H3K4me3")

data_for_plot <- lapply(histone_type, function(histone) {
    result <- lapply(variants_type, function(x) {
        get_data_for_plot_facet_H3K4me3_H3K9me3(paste0(histone, "/sliding_window_3D7_as_reference/relative.0.05/mutation_density/", x, "_mean_variant_count"))
    })
...
## 最后选定的两个组蛋白修饰
pdf(file = "histone_sliding_window_H3K9me3.pdf", width = 18, height = 10)
for (histone in c("H3K9me3")) {
...
pdf(file = "histone_sliding_window_H3K4me3.pdf", width = 18, height = 10)
for (histone in c("H3K4me3")) {
    p <- plot_mutation_density(data_for_plot = data_for_plot_global, histone_type = histone)
    print(p)
}
dev.off()

...
plot_and_cor_sliding_window_notitle <- function(data_for_plot = NULL) {
    ...
    p <- ggplot(data_for_plot %>% filter(source != "nopeak500")) +
...
plot_motif_averaged_flank <- function(data_for_plot = NULL, title = NULL) {
    ...
    p <- ggplot(data_for_plot) +
        ...
        labs(y = "6mA sites density") +
        ggtitle(title) +
        scale_x_discrete(labels = c("S1", "S2", "S3", "S4", "S5", "S6", "F1-10")) +
        scale_color_manual(values = color_value, labels = c("Motif", "Flank")) +
        theme_bw() +
        theme(
            panel.grid.major = element_blank(),
            panel.grid.minor = element_blank(),
...
sliding_window <- plot_and_cor_sliding_window_notitle(data_for_plot = data_for_plot)
...
each_site <- plot_motif_averaged_flank(data_for_plot = data_for_plot, title = "")
each_site$labels$y<-""
global<-sliding_window|each_site
pdf("3th.pdf", width = 20, height = 8)
print(global)
dev.off()
...
## 最终的sliding window的图
setwd("/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/6mA/jiang/3th/public/sliding_window")
pdf("6mA_distri_sliding_window_frac20_cov25.pdf", width = 10, height = 8)
...
setwd("/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/6mA/jiang/3th/public/motif_flank_pattern")
pdf("6mA_distri_motif_averaged_flank.pdf", width = 10, height = 8)
for (prefix in c("_FRACgt20_COVgt25")) {
    data_for_plot <- read_motif_and_averaged_flank_data(dataname = paste0("motif_flank_pattern_6mA_distri", prefix))

...
get_average_variant_count_for_each_motif_global_variants() {
    ...
    for var in $(echo "snps single_del" | tr " " "\n"); do
        mean_variant_count=$specific_mutation_density_dir/flank_${flank_site}.${var}_mean_variant_count
        ...
        for chrom in $(echo $autosomes | tr " " "\n"); do
            ...
            for peak_type in $(echo "peak nopeak" | tr " " "\n"); do
                for base_status in $(echo "A T C G" | tr " " "\n"); do
                    this_site_this_bases_count=$($seqkit subseq --quiet --bed $single_motif_output_dir/flank_${flank_type}/flank_${flank_site}/${chrom}.${peak_type}.bed $genome | $seqkit fx2tab -i -H -n -C "${base_status}" | grep -v "^#" | awk '{s+=$2} END{print s}')
                    this_site_this_bases_mutation_count=$(get_average_variant_count_for_chrom_base_count $chrom $single_motif_output_dir/flank_${flank_type}/flank_${flank_site}/${chrom}.${peak_type}.bed "${base_status}_${var}")
...
single_motif_output_dir="/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/across_species/two_outgroup_consistent/P_billcollinsi"
flank_sites=$(seq 1 10)
core_consistent_dir="/picb/evolgen/users/gushanshan/projects/malaria/dataAndResult/across_species/two_outgroup_consistent/P_billcollinsi/peak_nopeak"

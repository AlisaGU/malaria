...
read_peakbase_motif_and_flank_scale_mutation <- function(flank_length = NULL, motif = NULL, motif_seq = NULL, variants = NULL) {
    # read scale_factor
    setwd("../peak_base_mutation_density")
    scale_data <- read_scale_data(variants = variants)
    # read motif inside
    setwd(paste0("../", motif))
...
read_scale_data <- function(variants = NULL) {
    file_names <- dir()[grep("snps|indels", dir())]
...
setwd(global_dir)
pdf(file = "peakbase_motif_and_flank_global_variants.remove_close_overlap.pdf", height = 20, width = 20)
for (motif in c("GAMSAA", "GAHGAW")) {
    path <- paste0(global_dir, "/", motif)
    setwd(path)
    data_for_plot <- read_peakbase_motif_and_flank_mutation(flank_length = 10)
    p <- plot_motif(data_for_plot = data_for_plot, motif = motif, title = as.character(titles[motif]))
    print(p)
}
dev.off()
...
###########################################################################
variants <- c("global", "G", "A", "A_T_C", "A_T", "A_C", "G_C")
titles <- c("G,A,A/C,G/C,A,A", "G,A,A/T/C,G,A,A/T")
names(titles) <- c("GAMSAA", "GAHGAW")

setwd(global_dir)
pdf(file = "sclaed_peakbase_motif_and_flank_global_variants.remove_close_overlap.pdf", height = 20, width = 20)
for (motif in c("GAMSAA", "GAHGAW")) {
    path <- paste0(global_dir, "/", motif)
    setwd(path)
    data_for_plot <- read_peakbase_motif_and_flank_scale_mutation(
        flank_length = 10,
        motif = motif, motif_seq = as.character(titles[motif]), variants = variants
    )
    p <- plot_motif_scale(data_for_plot = data_for_plot, motif = motif, title = as.character(titles[motif]))
    print(p)
}
dev.off()
...
setwd(global_dir)
pdf(file = "scaled_by_propor_peakbase_motif_and_flank_global_variants.remove_close_overlap.pdf", height = 20, width = 20)
for (motif in c("GAMSAA", "GAHGAW")) {
    path <- paste0(global_dir, "/", motif)
    setwd(path)
...
variants <- c("G", "A")
titles <- c(
    "G,A,A/C,G/C,A,A", "G,A,A/T/C,G,A,A/T", "G,A,A,G,A,A", "G,A,A,G,A,T",
    "G,A,T,G,A,A", "A,T,C,A,T,C", "G,A,A/T,G,A,A/T", "G,A,A,C,A,A"
)
names(titles) <- c(
    "GAMSAA", "GAHGAW", "GAatGAat_1", "GAatGAat_2",
    "GAatGAat_3", "GAatGAat_4", "GAWGAW", "GAACAA"
)

setwd(global_dir)
ps <- list()
pdf(file = "scaled_by_propor_peakbase_motif_and_flank_global_variants.remove_close_overlap_all_snp.pdf", height = 10, width = 15)
for (motif in c("GAMSAA", "GAHGAW", "GAatGAat_1", "GAatGAat_3", "GAatGAat_4", "GAWGAW", "GAACAA")) {
    path <- paste0(global_dir, "/", motif)
    setwd(path)
    data_for_plot <- read_peakbase_motif_and_flank_scale_by_propor_mutation(flank_length = 10, motif = motif, variants = variants)
    # p <- plot_motif_scale_by_propor(data_for_plot = data_for_plot, motif = motif, title = as.character(titles[motif]))
...
#############################################################################
snp_site1 <- data_for_plot %>%
    filter(variant == "SNPs") %>%
    filter(site == "site1")
compare_means(value ~ 1,
    paired = FALSE, data = snp, method = "wilcox.test",
    group.by = "site", mu = 0
)

derived_1 <- function(data = NULL) {
    a <- wilcox.test(data$value, mu = 1, alternative = "greater")
    return(a$p.value)
}

ddply(data_for_plot, .(variant, site), derived_1)

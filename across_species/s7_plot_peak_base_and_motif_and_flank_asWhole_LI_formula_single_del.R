...
read_peakbase_motif_and_flank_scale_by_propor_mutation <- function(flank_length = NULL) {
    ...
    data_for_plot$variant <- gsub("single_del", "sDel", data_for_plot$variant)
    data_for_plot$variant_f <- factor(data_for_plot$variant)
...
read_scale_data <- function() {
    file_names <- dir()[grep("single_del", dir())]
...
read_each_site_motif_scale_by_propor <- function(site = NULL, scale_data = NULL, propor_data = NULL) {
    ...
    data <- lapply(file_names, function(x) {
        var <- gsub("_mean_variant_count", "", unlist(strsplit(x, ".", fixed = T))[2])
        ...
        data <- ""
        if (all(b[, 2] == 0)) {
            data <- b[, 3] / b[, 4] * (propor_data[[site]][, 3] + propor_data[[site]][, 4]) / unlist(scale_data[["C_single_del"]])
        } else if (all(b[, 4] == 0)) {
            data <- b[, 1] / b[, 2] * (propor_data[[site]][, 1] + propor_data[[site]][, 2]) / unlist(scale_data[["A_single_del"]])
        } else {
            data <- (propor_data[[site]][, 1] + propor_data[[site]][, 2]) * b[, 1] / b[, 2] / unlist(scale_data[["A_single_del"]]) +
                b[, 3] / b[, 4] * (propor_data[[site]][, 3] + propor_data[[site]][, 4]) / unlist(scale_data[["C_single_del"]])
        }
...
get_scale_factor_by_propor <- function(propor_data = NULL, scale_data = NULL) {
    A_T_C_G_peak_single_del_base_mutation_density <- data.frame(
        "A" = scale_data[["A_single_del"]], "T" = scale_data[["T_single_del"]],
        "C" = scale_data[["C_single_del"]], "G" = scale_data[["G_single_del"]]
    )
    result_single_del <- lapply(propor_data, function(x) {
        result <- as.data.frame(apply(x * A_T_C_G_peak_single_del_base_mutation_density, 1, sum))
        colnames(result) <- "scale_factor"
        return(result)
    })
    result <- list(single_del = result_single_del)
...
read_each_site_flank_scale_by_propor <- function(flank = NULL, scale_data = NULL, propor_data = NULL) {
    ...
    data <- lapply(file_names, function(x) {
        ...
        data <- ""
        if (all(b[, 2] == 0)) {
            data <- b[, 3] / b[, 4] * (propor_data[[flank]][, 3] + propor_data[[flank]][, 4]) / unlist(scale_data[["C_single_del"]])
        } else if (all(b[, 4] == 0)) {
            data <- b[, 1] / b[, 2] * (propor_data[[flank]][, 1] + propor_data[[flank]][, 2]) / unlist(scale_data[["A_single_del"]])
        } else {
            data <- b[, 1] / b[, 2] * (propor_data[[flank]][, 1] + propor_data[[flank]][, 2]) / unlist(scale_data[["A_single_del"]]) +
                b[, 3] / b[, 4] * (propor_data[[flank]][, 3] + propor_data[[flank]][, 4]) / unlist(scale_data[["C_single_del"]])
        }
        rownames(data) <- rownames(b)
...
pdf(file = "scaled_by_propor_peakbase_motif_and_flank_global_variants.remove_close_overlap_all_single_del.pdf", height = 10, width = 15)
data_for_plot <- read_peakbase_motif_and_flank_scale_by_propor_mutation(flank_length = 10)
p <- plot_motif_scale_by_propor(data_for_plot = data_for_plot %>% filter(variant == "sDel"), title = "GAWGAW")
print(p)
dev.off()
